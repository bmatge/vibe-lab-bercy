...
Deux démonstrateurs de services grand public basés sur les open data du ministère. Prix Contrôle Technique (carte choroplèthe, filtres multi-critères, observatoire statistique) et Tarifs Bancaires (wizard multi-étapes, comparaison côte à côte, export CSV). Les deux branchés sur l’API data.economie.gouv.fr.
...
Typage strict : TypeScript sur 5/8 projets, Pydantic côté Python. Tests : 1 800+ tests unitaires et d’intégration + E2E Playwright. CI/CD : GitHub Actions (linting, tests, build Docker, scan Trivy). Documentation : README détaillés, ADR, documentation technique générée.
...
Les 8 outils déjà produits couvrent les trois dimensions du design ministériel : design visuel (ChartsBuilder, thème LimeSurvey, organigrammes, cohérence de marque), design de service (comparateurs grand public, Écosystème, LeFouineur, parcours utilisateur) et design éditorial (Portail Eco Browser, LeFouineur contributeurs, ChartsBuilder rédacteurs).
...
Le Vibe Lab permet à la MIWEB de passer de l’ingénierie du web à l’intelligence du web. Intelligence éditoriale (Portail Eco Browser : connaissance fine du contenu publié), intelligence de gouvernance (Écosystème : pilotage par les données), intelligence du design (prototypage rapide, tests sur outils réels, itération continue) et intelligence de l’accessibilité (audit automatisé permanent, recommandations IA, contrôle qualité embarqué).
...
Étape 1 — Le Vibe Lab explore. Le Lab identifie un besoin, produit un prototype fonctionnel sur données publiques, teste avec les utilisateurs, valide le concept.
Étape 2 — La direction arbitre. Si le prototype est validé et que le besoin justifie un traitement sur données sensibles, le projet est transféré à BercyHub.
Étape 3 — BercyHub industrialise. BercyHub reprend le concept validé, le connecte aux SI métier et aux données sensibles, dans le cadre d’une homologation.
...
Niveaux de fichiers
- Niveau 1 — Fichiers de suivi simples : migration vers Grist en quelques heures
- Niveau 2 — Fichiers à formules complexes : 1 à 2 semaines, interface DSFR complète
- Niveau 3 — Fichiers d’agrégation multi-sources : 2 à 4 semaines, connecteurs API
- Niveau 4 — Fichiers à données sensibles : HORS PÉRIMÈTRE, signalés à BercyHub
...
PARTIE III
# Le cadre : gouvernance et garde-fous
## 11. Gouvernance
...
Phase 1 — Demande. Une direction identifie un besoin et le soumet via un canal unique. Le Lab vérifie les critères d’entrée (6 critères obligatoires).
Phase 2 — Évaluation. La grille de scoring (50 points, 6 critères pondérés) classe le projet dans le backlog.
Phase 3 — Prototypage. Sprint de 1 à 2 semaines. Livrable : prototype fonctionnel testable.
Phase 4 — Test et itération. Test utilisateur avec la direction sponsor. Itération rapide.
Phase 5 — Bilan. Le prototype est-il adopté ? Usage réel ? Valeur démontrée ?
Phase 6 — Décision. Trois issues : pérenniser, transférer à BercyHub, ou abandonner.
Instances
...
- Pas de projet sans sponsor métier identifié
- Pas de projet hors périmètre (données publiques ou agrégées uniquement)
- Maximum 5 projets actifs simultanément
- Abandon à 4 semaines sans usage
- Transparence totale : code sur GitHub, métriques dans le tableau de bord, bilans dans la revue trimestrielle
## 12. Grille d’évaluation
Critères d’entrée (tous obligatoires)
- Sponsor métier identifié (une direction ou un bureau porte le besoin)
- Données publiques ou agrégées (aucune donnée personnelle ou sensible)
- Hors périmètre BercyHub (pas de connexion à un SI métier)
- Pas de solution existante (pas de doublon avec un outil en service)
- Prototypable en ≤ 4 semaines (si plus, c’est un projet SI classique)
- Alignement avec les missions du Lab (outillage, pilotage, communication)
...
PARTIE IV
# La demande : moyens et calendrier
## 15. Moyens demandés
...
| 16 cas d’usage identifiés. 16 à 100 % dans le périmètre Vibe Lab.
0 interférence avec BercyHub. |
| --- |
...
| Structurer le Vibe Lab, c’est transformer une initiative personnelle en capacité institutionnelle.
C’est un investissement modeste pour un effet de levier considérable sur la production, le positionnement et la visibilité du ministère. |
| --- |
